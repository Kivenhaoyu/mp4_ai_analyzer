...
#include "util/frame/frame_guard.h"
...
//    
//    MediaFramePool yuvpool(10,width,height,AV_PIX_FMT_UYVY422);
//    MediaFramePool resizedpool(10,224,224,AV_PIX_FMT_RGB24);
...
//    if (!infer_engine.init(model_path)) {
...
//        FrameGuard<MediaFramePool> yuv_guard(yuvpool);
//        FrameGuard<MediaFramePool> resize_guard(resizedpool);
//        
...
////        if (!FrameConverter::convertCropResizeYuvToBgr(yuv_frame, resize_frame)) {
...
//        AIResult ai_result = infer_engine.infer(model_input, model_input_size);
...
//    renderer->init("AI Camera Window", width / 2, height / 2);
//    
//    MediaFramePool yuvpool(3,width,height,AV_PIX_FMT_YUV420P);
//    MediaFramePool rgbpool(3,width,height,AV_PIX_FMT_RGB24);
//    MediaFramePool resizedpool(3,224,224,AV_PIX_FMT_RGB24);
//    // 分配输出缓冲区（224×224×3=150528个float）
//    const int model_input_size = 224 * 224 * 3;
//    float* model_input = new float[model_input_size];
//    auto start_time = std::chrono::high_resolution_clock::now();
...
////        if (!.convertCropResizeYuvToBgr(yuv_frame, resize_frame)){
////            std::cerr << "第" << i << "帧转换失败：" << decoder.getErrorMsg() << std::endl;
////            continue;
////        }
//        
