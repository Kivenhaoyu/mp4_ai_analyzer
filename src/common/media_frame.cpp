...
bool VideoFrame::allocateBuffers() {
    ...
    switch (pix_fmt_) {
        ...
        {
            const int y_line_size = width_;
            const int uv_line_size = (width_ + 1) / 2; //向上取整
            const int y_size = width_ * height_;
            const int uv_size = uv_line_size * ((height_ + 1) / 2);
            
            uint8_t* y_data = new (std::nothrow) uint8_t[y_size]();
            uint8_t* u_data = new (std::nothrow) uint8_t[uv_size]();
            uint8_t* v_data = new (std::nothrow) uint8_t[uv_size]();
            ...
            std::memset(u_data, 128, uv_size);
            std::memset(v_data, 128, uv_size);
            
            data_ = {y_data,u_data,v_data};
            linesize_ = {y_line_size, uv_line_size, uv_line_size};
            return true;
        }
        case PixelFormat::BGR24:
        case PixelFormat::RGB24:{
            const int line_size = width_*3;
            const int size = line_size * height_;
        ...
        case PixelFormat::NV12: {
            const int y_line_size = width_;
            const int uv_line_size = width_;
            const int y_size = y_line_size * height_;
            const int uv_size = uv_line_size * (height_ / 2);
            
            uint8_t* y_data = new (std::nothrow) uint8_t[y_size]();
            uint8_t* uv_data = new (std::nothrow) uint8_t[uv_size]();
            ...
            std::memset(uv_data, 128, uv_size);
            
            data_ = {y_data, uv_data};
            linesize_= {y_line_size, uv_line_size};
            return true;
        }
        default:
            return false;
...
void VideoFrame::freeBuffer() {
    if (!isShallowCopy()) {
        for(uint8_t* ptr : data_) {
            delete [] ptr;
        }
    }
    data_.clear();
...
bool AudioFrame::allocateBuffers() {
    freeBuffer();
    
    int bytes_per_sample = 0;
    switch (sample_format_) {
        case SampleFormat::S16: bytes_per_sample = 2; break;
        case SampleFormat::S32: bytes_per_sample = 4; break;
        case SampleFormat::FLT: bytes_per_sample = 4; break;
        case SampleFormat::U8:  bytes_per_sample = 1; break;
        default: return false;
    }
    
    data_size_ = nb_samples_ * channels_ * bytes_per_sample;
    data_ = new (std::nothrow) uint8_t[data_size_]();
    return data_ != nullptr;
}

void AudioFrame::freeBuffer() {
    if (!isShallowCopy() && data_) {
        delete[] data_;
        data_ = nullptr;
        data_size_ = 0;
    }
